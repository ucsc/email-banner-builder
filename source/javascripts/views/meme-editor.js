...
MEME.MemeEditorView = Backbone.View.extend({
  ...
  buildForms: function() {
    ...
    if (d.bannerSizeOpts && d.bannerSizeOpts.length) {
      $('#banner-width').append(buildOptions(d.bannerSizeOpts)).show();
    }
  ...
  events: {
    ...
    'change #banner-width': 'onBannerSize',
  ...
  onBannerSize: function() {
    var selectedWidth = this.$('#banner-width').val();
    this.model.set('width', this.$('#banner-width').val());
    if (selectedWidth === '640') {
      this.model.set('height', 325);
    } else {
      this.model.set('height', 200);
    }
  },  
